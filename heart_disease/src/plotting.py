...
def plot_pie_chart(df, column):
    ...
    heart_disease[column].value_counts().sort_index().plot(
        kind="pie",
        autopct="%1.1f%%",
        radius=0.8,
    ...
    no_heart_disease[column].value_counts().sort_index().plot(
        kind="pie",
        autopct="%1.1f%%",
        radius=0.8,
        wedgeprops={"linewidth": 3.0, "edgecolor": "white"},
...
def look_for_correlations(df):
    ...
    sns.heatmap(
        num_df.corr(), cmap="RdYlGn", vmin=-0.5, vmax=0.5, annot=True, fmt=".2f"
    )
